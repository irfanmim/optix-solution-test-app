...
class ListOfMachine extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            isLoading : false,
                        ...
                        <TableBody>
                            {this.state.data.map(n => (
                                <TableRow key={n.id} style={{textDecoration: 'none'}}>
                                    ...
                                    <TableCell component="th" scope="row">{n.enabled}</TableCell>
                                    <TableCell component="th" scope="row">{n.type}</TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
                    { this.state.data.length === 0 ? <Typography style={{textAlign: "center"}}>no machine</Typography> : <div></div>}
                </Paper>
            </div>
        )
    }
...
export default withStyles(styles)(ListOfMachine);